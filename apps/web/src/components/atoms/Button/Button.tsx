...
export function Button({ variant = 'primary', fullWidth = false, children, className = '', ...props }: ButtonProps) {
  const base = 'py-3 px-6 rounded-lg font-semibold text-base transition-all duration-200 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed'
  const fullWidthClass = fullWidth ? 'w-full' : ''

  const variants: Record<ButtonVariant, string> = {
    primary: 'bg-accent-green text-btn-text hover:bg-accent-green-hover active:scale-[0.98]',
